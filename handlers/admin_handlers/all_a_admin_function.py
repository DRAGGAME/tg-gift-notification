...
from keyboards.menu_fabric import FabricInline, InlineMainMenu, InlineAdminMenu
...
@router_for_admin.callback_query(InlineAdminMenu.filter(F.action == "exit"))
async def exit_admin(message: Message, state: FSMContext):
    ...
    await message.answer("Вы вышли из администратора")
...
async def login(message: Message, state: FSMContext):
    ...
    password = await sqlbase_for_admin_function.execute_query('SELECT password_admin FROM settings_table;')
...
@router_for_admin.callback_query(InlineAdminMenu.filter(F.action == "UpdPassword"))
...
async def new_password(message: Message, state: FSMContext):
    ...
    if alt_newpassword is None:  # Первый ввод пароля
        ...
        if message.text.lower() == 'stop':
            ...
            query = 'UPDATE settings_table SET password_admin = $1 WHERE id = 1;'
